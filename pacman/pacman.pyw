...
# parser.add_argument('-m','--mode', help='Train / Run', required=False, default="Run")
...
while True: 
    ...
    if thisGame.mode == 1:
        ...
        if aux_step%FRAMES_FOR_STEP==0:
            print("--------------------------------------------------")
            action = learn.getAction(pygame.surfarray.array3d(window))
        ...
        for i in range(0, 4, 1):
            ghosts[i].Move()
        thisFruit.Move()
        #print("Step {} concluded".format(global_step))
        ...
        if thisGame.modeTimer == 90:
            ...
            #print("Step {} concluded with reward: {}".format(global_step,reward))
            print("Died in step {} with reward {}".format(global_step,reward))
    elif thisGame.mode == 3:
        # game over
        #CheckInputs()
        thisGame.StartNewGame()
        ...
        if thisGame.modeTimer == 30:
            learn.storeMem(pygame.surfarray.array3d(window),75,action)
            thisGame.SetMode( 1 )
        ...
        if thisGame.modeTimer == 60:
            ...
            learn.storeMem(pygame.surfarray.array3d(window),reward,action)
            #print("Step {} concluded with reward: {}".format(global_step,reward))
            print("Won game in step {} with reward {}".format(global_step,reward))
            thisGame.SetMode( 7 )
    ...
    if thisGame.mode==1 and action!=None:
        ...
        aux_step+=1
        #print("Aux_Step {} concluded with reward: {} / Global_Step: {}".format(aux_step,reward,global_step))
